...
def test_load_settings_uses_default_instance_directory(
    ...
    monkeypatch.delenv("SINGNOTE_SHARED_PASSCODE", raising=False)
    ...
    assert settings.shared_passcode is None
...
def test_load_settings_accepts_environment_overrides(
    ...
    monkeypatch.setenv("SINGNOTE_SHARED_PASSCODE", "teacher-pass")

    settings = load_settings()

    assert settings.environment == "test"
    assert settings.data_dir == tmp_path / "custom-data"
    assert settings.shared_passcode == "teacher-pass"


def test_bootstrap_application_creates_data_directory_and_database(
    monkeypatch,
    tmp_path: Path,
) -> None:
    """Bootstrap should prepare persistence dependencies."""
    monkeypatch.chdir(tmp_path)
    ...
    assert app.repository.list_songs() == []
